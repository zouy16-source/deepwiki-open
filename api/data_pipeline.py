...
# Generic markers left by code generators in a file header/annotation. Kept
# specific enough that they mean "the whole file is generated" — deliberately NOT
# the bare word "auto-generated", which matches the ubiquitous Eclipse catch-block
# comment ("// TODO Auto-generated catch block") in otherwise hand-written code.
_GENERATED_MARKERS = (
...
def is_generated_boilerplate(relative_path: str, content: str) -> bool:
    """Heuristic: True if the file is machine-generated boilerplate not worth embedding.

    Codegen-heavy repos (e.g. MyBatis) can be dominated by auto-generated criteria/
    POJO classes that carry near-zero wiki/RAG value but blow up the embedding index
    (on galaxy-waybill the MyBatis *Example criteria classes alone are ~40% of all
    Java lines). Skipping them massively cuts indexing time. Two signals:

    1. MyBatis Generator *Example criteria classes — a `*Example.java` whose body is
       the generated `oredCriteria`/`BaseExample` addCriterion(...) builder. These
       have no header marker, so they're matched structurally.
    2. A generic generator header/annotation near the top of the file.
    """
    name = os.path.basename(relative_path)
    head = content[:4000].lower()

    # 1. MyBatis Generator *Example criteria classes (structural — no header marker)
    if name.endswith("Example.java") and ("oredcriteria" in head or "baseexample" in head):
        return True

    # 2. Generic generated-file markers
...
def read_all_documents(path: str, embedder_type: str = None, is_ollama_embedder: bool = None,
    ...
    if embedder_type is None and is_ollama_embedder is not None:
        embedder_type = 'ollama' if is_ollama_embedder else None
    documents = []
    skipped_generated = 0  # count of machine-generated files skipped (logged at end)
        ...
        for file_path in files:
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    # Skip machine-generated boilerplate (MyBatis *Example criteria
                    # classes, files with a generator header) — near-zero wiki value,
                    # dominates the embedding index on codegen-heavy repos.
                    if is_generated_boilerplate(relative_path, content):
                        skipped_generated += 1
    ...
    if skipped_generated:
        logger.info(f"Skipped {skipped_generated} machine-generated file(s) (MyBatis *Example / generated headers)")
...
class ConcurrentToEmbeddings(adal.DataComponent):
    ...
    ``num_batches * per_request_latency`` — e.g. ~1100 batches * ~0.55s ≈ 11 min,
