...
/**
 * GitLab 仓库列表(搜索 + 表格 + 分页 + 生成/查看)。
 * 从原 /wiki/catalog 页抽出,供首页「项目列表」与 catalog 路由共用。
 */
export default function GitlabProjectList({ className = '' }: { className?: string }) {
            ...
            {!loading && rows.map(({ p, isGenerated, viewHref, genHref, nested }) => (
              <tr key={p.pathWithNamespace} className="hover:bg-[var(--card-bg)] align-top">
                <td className="px-4 py-3 font-mono">
                  {p.webUrl ? (
                    <a href={p.webUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--link-color)] hover:underline">{p.pathWithNamespace}</a>
                  ) : (
                    <span className="text-[var(--foreground)]">{p.pathWithNamespace}</span>
                  )}
                </td>
